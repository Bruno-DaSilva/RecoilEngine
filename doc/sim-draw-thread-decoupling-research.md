...
Model tiers — **[F] = Fable**, **[O] = Opus**. Rule of thumb: Fable where the work is design-setting (first of a pattern), proof-carrying (sync audits), or semantic-edge-heavy (lifetime, ordering, lazy→eager changes); Opus where a pattern already exists, the spec here is complete, and the mechanical gates (A/B, demo-resim, diff gate) catch errors. Nothing before PR 19 changes game-visible behavior. Verification: A/B pixel gate, master-demo resim gate (above), benchmark suite.
...
13. [F] (T4) Deferred-deletion epoch + freed-slot poisoning — the sharpest lifetime edge in the plan; wants the strongest model *and* the strongest review.
