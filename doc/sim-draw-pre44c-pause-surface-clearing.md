...
**Escalation:** a *distinct* sim-thread camera **write** — `camera->SetRotY(…)` at GroundMoveType.cpp:3324 (`UpdateDirectControl`, FPS/direct-unit-control) — races the draw thread's camera use. Narrow (only under player direct-control) and headful-only. **Operator call needed:** is direct-unit-control a supported combination under the split? If yes, the `SetRotY` should be gated/deferred to the draw side or the boundary; if direct-control + split is out of scope, document and leave. Not fixed here (design ambiguity, ESCALATION rule).
