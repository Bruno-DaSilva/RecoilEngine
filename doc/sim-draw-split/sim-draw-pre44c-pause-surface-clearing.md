...
| 9 | `SendCommands` | LuaUnsyncedCtrl.cpp:603 | on `Spring.SendCommands` (Lua) | console-action executors (see §4.5-SendCommands) | **SERVED (Prereq E, LANDED 2026-07-09)** — per-action park scoping via `IUnsyncedActionExecutor::touchesSimState` (default-park; DRAW-UI/NET-SEND marked sim-safe); full-batch park removed. `LUA_SEND_COMMANDS` ~10k → 1/game headful. Design `doc/sim-draw-pr44-prereqE-sendcommands-park-scoping.md`. |
...
Disposition (this audit PR): kept the conservative full-batch park; the headful measurement (≈10k engages/game — stock BAR widgets call `SendCommands` ~per draw frame) reclassified it from "rare" to a per-frame headful park, escalated as a needed 44b prerequisite.

**FOLLOW-UP LANDED — Prereq E (2026-07-09, `epoch-integration`; design `doc/sim-draw-pr44-prereqE-sendcommands-park-scoping.md`).** The per-action scoping is implemented: `IUnsyncedActionExecutor` gains a `touchesSimState` flag (default TRUE = park, conservative), the DRAW-UI/NET-SEND families are marked FALSE at registration, and `CGuiHandler::RunCustomCommands` parks only when the resolved executor touches sim (unresolved/local/unknown actions keep the park). The full-batch park in `SendCommands` was removed. Gate: flag-OFF resim both replays 0 DESYNC; flag-ON headful ×4 0 desync/0 lua/0 nil-storm; **`LUA_SEND_COMMANDS` ~10055/10376 (rosetta), ~9722/9654 (atg) → 1 in all four runs**. Only the SIM-POKE minority (`DumpState`/`ViewSelection`/`Give`/`Destroy`/particle-limits/team/select/group/speed/etc.) still parks (input-gated, ≈0 in steady state).
