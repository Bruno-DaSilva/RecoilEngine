...
- 2026-07-31 — **First RenderDoc capture attempt: it hooks, it refuses. And the post-conversion frame is 13,517 calls / 15.1% rejected (was 66,794 / 84.7%).**
  ...
  - **The obvious hypothesis is not yet proven.** RenderDoc documents core 3.2+ only, and the engine runs `4.6 (Compatibility Profile)` — but the control experiment could not be completed: forcing a core context crashes the engine at 0.4 s in the Mesa driver, since startup itself is still fixed-function. That crash is the honest state of the port, not a bug to fix in isolation.
  - **`ForceCoreContext=1` is a silent no-op on its own** (engine finding, worth fixing independently): `CreateGLContext` requests `GLContextMajorVersion`/`MinorVersion`, which default to **3.0**, and a core profile needs ≥3.2 — so the driver hands back a compatibility context and the log still says `(Compat)`. Getting core requires raising those two knobs as well (4.5 works).
  - **Next lead for the refusal:** the trace shows the first context created through `glXChooseVisual` + plain `glXCreateContext` (calls 7–13, the 32×32 probe), not `glXCreateContextAttribsARB`. RenderDoc does not support legacy unversioned GLX contexts, and if that probe context is the one it latches onto, the profile question is a red herring.
