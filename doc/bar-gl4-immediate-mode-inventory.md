...
## Whole-frame A/B "test mode" (config `GLFrameABCompare`)

A validation harness that renders each sim state twice — on two consecutive real display frames with the modern-GL paths toggled off then on — and compares the backbuffer. Runs the game at half fps. Unlike an in-frame re-render (which breaks drawFrame-throttled widgets and screen-grab blur), each pass is a distinct real frame, so widgets rebuild normally. To make the two frames draw identical content, the duplicate (2nd) pass replays the reference frame's entropy + clocks, all pinned *inside* `UpdateUnsynced` before the per-particle draw-pos / per-draw UBO bake:

- **sim** frozen (no `SimFrame` on the duplicate — `ClientReadNet` skipped)
- **`guRNG` + Lua `math.random`** snapshot/restore (`spring_lua_unsynced_rand_save/restore_state`) — covers UBO `rndVec3` (shader dither) and FX spawn/draw jitter
- **`Spring.GetTimer`/`GetTimerMicros`** pinned; **`os.clock`** replaced in unsynced states and pinned (`LuaLibs::OpenUnsynced` → `LuaUnsyncedRead::OsClock`) — wind-turbine etc.
- **`grTime`** pinned (UBO `timeInfo.drawSeconds`)
- **`drawFrame` + `timeOffset` + `lastFrameTime`(=0) + `gameTime`** pinned inside `UpdateUnsynced`, before the widget `:Update`s and `worldDrawer.Update()`'s `GetDrawPos(timeOffset)` bake

Control pairs (every other pair is legacy↔legacy) build a noise mask that excludes counter-driven drift (TAA/exposure/SSAO). Result on the BAR fighter benchmark: **median whole-frame divergence ~76–350 px / 3.69M (< 0.01 %)**, most combat frames byte-identical. `AB_FORCE_LEGACY=1` (env) forces both passes legacy → same residual, proving the divergence is inherent per-frame non-determinism, **not** the migration (the modern toggle provably doesn't touch the C++ FX/particle path).

**Duplicate-pass protocol.** The last residuals are BAR widgets that advance private animation state in their draw callin (`self.fade += n`) — unreachable by any engine input-freeze. Engine exposes **`Spring.GetABDuplicatePass()`** (true only on the 2nd pass); a stateful widget guards its advance with `if not Spring.GetABDuplicatePass() then ... end` and redraws the reference phase → byte-identical. Validated on the wind turbine (`gui_top_bar`, `windRotation`) → clean frames hit exactly 0. Remaining candidates: `gui_info` unit panel (+ its guishader blur — note it uses deprecated `gl.CreateList`, so its diff may be a *real* modern-backend finding), event-message fade. Alternative that needs no game-Lua changes: accumulate the legacy↔legacy mask over a session and flag only out-of-mask diffs.
