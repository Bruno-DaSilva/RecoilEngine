...
- 2026-07-31 — **State at 42, with the next targets costed from the live census.**
  ...
  - **What each remaining cheap function actually costs** (reachable sites, from `rdoc_site_census.sh`):
    - **Client-array cluster — best ratio on the board: 7 functions** (`glEnableClientState`, `glDisableClientState`, `glVertexPointer`, `glColorPointer`, `glNormalPointer`, `glTexCoordPointer`, `glClientActiveTexture`) from **2** call sites: `CLineDrawer::DrawAll` and `S3DModelVAO::Bind/UnbindLegacyVertexAttribsAndVBOs`. Blocker found: `CLineDrawer` feeds `glColorPointer(4, GL_FLOAT, …)` and **every** `VA_TYPE_*` colour is 8-bit `SColor`, which rounds each component by up to 0.5 LSB — the documented visible-delta class. A float-colour vertex type is a prerequisite, not an optional refinement.
