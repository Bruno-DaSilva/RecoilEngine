...
- 2026-07-04 — **Coverage breadth (divergence-plan step 2) — dev-content gates all 0/0; three engine parity-bug families found and fixed by the replay content.** Ran the gate beyond fightertest: a Supreme Isthmus 24-player replay (draft mode, reflection/refraction, chat/playerlist churn), a local Supreme Isthmus water test (BumpWater reflections + PiP + camera waypoints over water + buildmenu/build orders) and an idle base-building start. Engine fixes (all found by the gate, localized via the null test + apitrace pass-diff + dump PNGs, exactly per the documented recipe):
  ...
  - **Results:** fightertest 279/279, water-map 644/644, idle-start 410/410 — control=0 AND signal=0 (incl. pregame). **Replay residual (all release-archive path, all absent from dev gates, next session's targets):** (a) minimap camera-box class — legacy `(186,170,255)` vs modern `(186,170,37)`, blue-channel-only, ~600–1300 px at f≈180–2020, max 218, per-call compare clean ⇒ inter-draw state propagation, suspect pip-minimap; (b) playerlist icon ~160 px max ~210 (14×16 icon at x≈2179–2193), texture-enable fix candidate — NOT yet re-validated on replay; (c) "Starting in N" countdown digit content differing p2 vs p3 (~600 px bursts, modern-linked, widget not identified — string absent from the BAR repo, suspect lobby/console-rendered); (d) late-game signals up to 16k px past f≈3000 (unlocalized; camera-over-combat content). Null test [L,L,L,L] on the replay: control ~0 (music/playerlist classes fixed by shadows), signal 0 — so ALL residuals are modern-linked.
...
2. ~~**Coverage breadth**~~ done 2026-07-04 (see changelog): dev-content gates (fightertest / Supreme Isthmus water test / idle start) all 0/0 after three engine parity-fix families + the second BAR guard batch. Fast local loop established (~1 min/gate cycle via `test/gl-ab-compare/` scripts — replays cost ~10 min because the recorded lobby replays at 1x, so iterate locally and confirm on the replay). **Open replay-only residual tail** (release-archive path; signatures in the changelog): minimap camera-box blue-channel class, playerlist icon class (texture-enable fix pending replay re-validation), "Starting in N" digit, unlocalized late-game classes. Next session: reproduce the box class locally (suspect pip-minimap mode), re-validate the icon class, identify the "Starting in" drawer.
