...
- 2026-07-31 — **Baseline corrected to 47 (the meter was measuring itself), the meter now resolves call SITES, and the first batch lands: 47 → 43, pixel-clean.**
  ...
  - **Note the gate's blind spot, which now matters.** `run_gate.sh` compares modern-vs-legacy *within one build*. An engine-side change that moves both passes together — removing `CShadowHandler`'s `GL_FLAT`, say — leaves it reading 0/0 while pixels changed against the previous build. Batch A did not need a cross-build gate because its changes are no-ops by construction, but anything that alters real GL state will, and there is no such gate today.
