...
- 2026-07-31 — **State at 41 / 407 reachable sites. Phase 0's precondition is MET for engine draws; what blocks the rest is content shaders reading fixed-function builtins.**
  ...
  - **The startup group is blocked on VERIFICATION, not on analysis — and `ClearScreen` is not as dead as it looks.** `glColor3f`'s only 3 reachable sites are all `ClearScreen()`, from `SpringApp::Init`, `CPreGame::Draw` and `CLoadScreen::Draw`. Two things make this harder than the other "sets the GL default" cases:
