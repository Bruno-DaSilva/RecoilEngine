...
### Follow-ups landed the same night (all gate-validated: 3 local gates 0, replay signal 0 on 4,7xx frames each)

- `e4a930b52a` — **font capture**: gl.Text / font:Print / font:Begin/End / color setters inside a capturing body are recorded as Lua-level FontCmds (shared_ptr keeps the font alive) and replayed through the LIVE font renderer — text-bearing lists no longer materialize, and glyphs use the CURRENT atlas instead of stale baked texcoords (a long-standing real-dlist defect). FONT_BUFFERED prints keep the normal path; inside open compiles the font's recordable path preserves real-list semantics.
- `7bcc1145b0` — **font-renderer dlist churn**: HandleTextureUpdate recompiled the texture-space glScalef list on every print batch (~190 glNewList/frame); now only on actual atlas resize.
- `396df03efa` — **oversized-capture perf guard**: captures with >4096 stream verts become real GL lists at creation (they'd take the exact-legacy replay anyway; driver-side replay is strictly better).

**Measured pure-modern frame after all of the above** (watertest, LuaModernGLBackend+LuaCommandLists+RenderBufferUseMVPUniform+FontUseMVPUniform): `glNewList` = **0** (was ~187/frame), `glCallList` = 4 (remaining materialized lists). The dominant remaining legacy class is LIVE dense-MV gl.BeginEnd world content (BAR reclaim-field hull fills, ~120k glVertex3f+glColor4fv per frame) plus glPushAttrib ~370 (font PushGLState + widget attrib pushes) plus the FF matrix/state scaffolding — all pinned on the FF-deletion arc (#5/#6): while the FF comparator exists, dense-MV/mirror-fed modern draws cannot hold byte-parity (measured composition + glRotatef-trig ULP laws), so these stay exact-legacy by design.
...
Resolution: modern immediate flushes and the RenderBuffer uMVP hook engage only under a screen-aligned modelview (no rotation/shear terms — covers virtually all BAR UI); dense-MV draws keep the exact legacy path. (Superseded in part by Constraint #1 in the inventory: the FF pipeline is never deleted, because non-BAR legacy games must stay frame-perfect — so bit-parity against FF never stops being a requirement for unconverted content. Dense-MV BAR content is freed only if its *own* draws are converted, not by FF going away.) The immediate backend's matrices come from the glGetFloatv bridge (the mirror re-derives glRotatef trig, ULP-differs); the mirror stays shadow-verified for Phase 2.
