...
The main loop is `Update → Draw`, repeating — see the diagram and table below for the per-phase breakdown. Each iteration first drains any queued sim-frame packets (0..N per iteration), then renders one draw frame. `CGame::Update` dispatches `SimFrame()` calls as `NETMSG_NEWFRAME` packets arrive; `CGame::Draw` runs the unsynced update phase and then renders. The sim burst is capped at ~500 ms (`minDrawFPS`) so draw always gets to run, and by default it's all one thread — sim and rendering are **not concurrent**; parallelism only happens *inside* a phase.

**The sim|draw thread split (`SimDrawSplit=1`, default off, PR 27b):** with the flag on, a dedicated sim thread runs `ClientReadNet → SimFrame`s (same code, same consumption budget) while the main thread keeps SDL/GL/draw/unsynced Lua. They meet once per draw frame: `CGame::Draw` parks the sim at a frame edge, runs `SimDrawBarrier` plus the drawer extraction with the sim quiescent, then releases; the render passes and Lua draw callins overlap the next sim burst, reading only snapshot/drawer-owned data (the split contract). Backpressure holds the sim ≤1 frame past the last published boundary during interpolated play and free-runs under fast-forward/catch-up. Frame time goes from `Σ(simFrames) + draw` to `max(sim, draw)` plus the boundary cost. See `doc/sim-draw-thread-decoupling-research.md` (the "Landed architecture" section) for the boundary contract and the measured numbers.
