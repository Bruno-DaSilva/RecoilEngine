...
void main()
{
   recoil_fragColor.a = 1.0; //note: only rendered with blending iff !opt_refraction

//#define dbg_coastmap
#ifdef dbg_coastmap
    recoil_fragColor = vec4(texture2D(coastmap, texCoords[0].st).g);
    return;
#endif
  ...
  // REFRACTION
   ...
   // If the distorted coordinate is closer to the screen in Z, then set the mixback to 1.0
    ...
    recoil_fragColor.rgb = mix(refrColor, waterSurface, 0.1 + surfaceMix * 0.100);
#else
    recoil_fragColor.rgb = waterSurface;
    recoil_fragColor.a   = surfaceMix + specular;
#endif
  ...
  // CAUSTICS
    if (waterdepth > 0.0) {
      ...
      recoil_fragColor.rgb += caust * caustBlend * CausticsStrength;
  #else
      recoil_fragColor.a *= min(waterdepth * 4.0, 1.0);
      recoil_fragColor.rgb += caust * (1.0 - waterdepth) * 7.5 * CausticsStrength;
  #endif
    }
  ...
  // SHORE WAVES
    recoil_fragColor.rgb += shadowOcc * GetShorewaves(coast, octave, waterdepth, invwaterdepth);

  // REFLECTION
    // Schlick's approx. for Fresnel term
    float fresnel = 0.0;
    vec3 reflColor = GetReflection(angle, normal, fresnel);
    recoil_fragColor.rgb = mix(recoil_fragColor.rgb, reflColor, fresnel * shallowScale);
