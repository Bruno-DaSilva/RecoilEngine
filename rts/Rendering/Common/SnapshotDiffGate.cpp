...
void SnapshotDiffGate::CheckBoundary()
{
	...
	for (size_t i = 0; i < maxUnits; ++i) {
		...
		// PR 38b tri-state: only ACTIVE rows have a live counterpart to compare;
...
void SnapshotDiffGate::CheckProjectileRows()
{
	...
	for (size_t id = 0; id < slots; ++id) {
		if (rows.valid[id] != SimSnapshotValid::ACTIVE || liveSeen[id] != 0) // PR 38b: skip DEAD_THIS_BATCH (no live counterpart)
			continue;
...
void SnapshotDiffGate::CheckWeaponRows()
{
	...
	for (size_t i = 0; i < maxUnits; ++i) {
		...
		if (u == nullptr || rows.valid[i] != SimSnapshotValid::ACTIVE) // PR 38b: only cross-check ACTIVE rows
			continue;
